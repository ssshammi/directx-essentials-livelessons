...
namespace Rendering
{
	...
	TransparencyMappingDemo::TransparencyMappingDemo(Game& game, Camera& camera)
		: DrawableGameComponent(game, camera), mVertexShader(nullptr), mInputLayout(nullptr), mPixelShader(nullptr), mVertexBuffer(nullptr),
		  ...
		  mWorldMatrix(MatrixHelper::Identity), mDirectionalLight(nullptr), mVertexCount(0), mColorTexture(nullptr), mTransparencyMap(nullptr),
		  mTrilinearSampler(nullptr), mFogStart(75.0f), mFogRange(40.0f),
	...
	TransparencyMappingDemo::~TransparencyMappingDemo()
	{
		...
		ReleaseObject(mTransparencyMap)
	...
	void TransparencyMappingDemo::Initialize()
	{
		...
		ThrowIfFailed(DirectX::CreateWICTextureFromFile(mGame->Direct3DDevice(), mGame->Direct3DDeviceContext(), textureName.c_str(), nullptr, &mTransparencyMap), "CreateWICTextureFromFile() failed.");
	...
	void TransparencyMappingDemo::Draw(const GameTime& gameTime)
	{
		...
		static ID3D11ShaderResourceView* PSShaderResources[] = { mColorTexture, mTransparencyMap };
